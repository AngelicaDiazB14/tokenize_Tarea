...
fn subtract_token_type(token: &TokenType) -> usize {
    match token {
        TokenType::Digit(_) | TokenType::Ident(_) | TokenType::Other(_) => 7,
        TokenType::Char(_) => 6,
        TokenType::Op(_) => 4,
    }
}
...
fn add_columns(token: &Token) -> usize{
    ...
    token_length - subtract_token_type(&token.tipo)
}
