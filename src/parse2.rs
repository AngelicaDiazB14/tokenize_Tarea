...
#[derive(Debug)]
pub enum ASTNode {
    Let(Box<ASTNode>, Box<ASTNode>), //LISTO en DOT -- Version del Profe
    Const(String, Box<ASTNode>),
    Var(String, String), //LISTO en DOT -- Version del Profe
    Func(String, Vec<ASTNode>, String, Box<ASTNode>), //LISTO en DOT -- Version distinta del Profe
    Proc(String, Vec<ASTNode>, Box<ASTNode>),  // NO ESTOY SEGURO DE SI DEBE IMPLEMENTARSE en DOT
    Type(String, Box<ASTNode>), // NO ESTOY SEGURO DE SI DEBE IMPLEMENTARSE en DOT
    Assign(Vec<String>, Box<ASTNode>),
    If(Box<ASTNode>, crate::TokenType, Box<ASTNode>, crate::TokenType, Box<ASTNode>),
    While(Box<ASTNode>, Box<ASTNode>), // NO ESTOY SEGURO DE SI DEBE IMPLEMENTARSE en DOT
    Call(String, Vec<ASTNode>),
    Expression(Box<ASTNode>), // NO ESTOY SEGURO DE SI DEBE IMPLEMENTARSE en DOT
    Identifier(String), //LISTO en DOT -- Version distinta del Profe
    Number(i64), //LISTO en DOT -- Version del Profe
    Char(char), //LISTO en DOT -- Version del Profe
    Operator(String, Box<ASTNode>, Box<ASTNode>),
    Declaration(Vec<ASTNode>), //LISTO en DOT -- Version del Profe
    Command(Vec<ASTNode>), //LISTO en DOT -- Version del Profe
}
impl ASTNode {
    ...
    fn to_custom_dot(&self) -> String {
        match self {
            ASTNode::Let(declarations, command) => format!( //LISTO en DOT -- Version del Profe
                "\"let\"->\"declaration\";\n {};\n\"let\"->\"command\";\n{};\n)",
                declarations.to_custom_dot(),
                command.to_custom_dot()
            ),
            ASTNode::Const(name, value) => format!(
                "const(\n   name(\"{}\"),\n   value(\n      {}\n   )\n)",
                name,
            ...
            ASTNode::Var(name, type_name) => format!( //LISTO en DOT -- Version del Profe
                "\"var\"->\"vname\";\n\"vname\"->\"{}\";\n\"var\"->\"typeName\";\n\"typeName\"->\"{}\";\n",
                name, type_name
            ),
            ASTNode::Func(name, params, return_type, body) => format!(
                ...
                name,
                params.iter()
                    .map(|p| format!("\"'[|]'\"->\"{}\";\n{}", p.to_custom_dot().split('"').nth(1).unwrap_or(""), p.to_custom_dot()  ))
                    .collect::<Vec<_>>()
                    .join("\n"),
                return_type,
                body.to_custom_dot().split('"').nth(1).unwrap_or(""),
                body.to_custom_dot()
            ),
            ASTNode::Assign(names, expr) => format!(
                "assign({:?}, {})",
            ...
            ASTNode::If(cond, _, then_branch, _, else_branch) => format!(
                "ifCmd(\n   cond({}),\n   then(\n      {}\n   ),\n   else({})\n)",
                cond.to_custom_dot(),
                then_branch.to_custom_dot(),
                else_branch.to_custom_dot()
            ),
            ASTNode::Call(name, params) => format!(
                "\"call\"->\"{}\";\n\"call\"->\"'[|]'\",params([{}]))",
                name,
                params.iter()
                    ...
                    .collect::<Vec<_>>()
                    .join("\n")
                //params.iter().map(|p| p.to_custom_dot()).collect::<Vec<_>>().join(",")
            ),
            ASTNode::Operator(op, left, right) => format!(  //LISTO en DOT -- Version distinta del Profe
                "\"op\"->\"{}\";\n\"op\"->\"{}\";\n\"op\"->\"{}\";\n {};\n{};\n)",
                op,
                left.to_custom_dot().split('"').nth(1).unwrap_or(""),
                right.to_custom_dot().split('"').nth(1).unwrap_or(""),
                left.to_custom_dot(),
                right.to_custom_dot()
            ),
            ASTNode::Number(num) => format!("\"num\"->\"'{}'\";\n", num), //LISTO en DOT -- Version del Profe
            ASTNode::Char(c) => format!("\"char\"->\"'{}'\";\n", *c as u8), //LISTO en DOT -- Version del Profe
            ASTNode::Identifier(name) => format!("\"ref\"->\"'{}'\";\n", name), //LISTO en DOT -- Version distinta del Profe
            ASTNode::Declaration(declarations) => format!( //LISTO en DOT -- Version del Profe**
                "\"declaration\"->\"'[|]'\";\n{}",
                declarations.iter()
                    .map(|d| format!("\n\"'[|]'\"->\"{}\";\n{}\n", d.to_custom_dot().split('"').nth(1).unwrap_or("") , d.to_custom_dot()  )) //SE BUSCA QUE DECLARATION APUNTE A LOS HIJOS
                    .collect::<Vec<_>>()
                    .join("\n")
            ),
            ASTNode::Command(commands) => format!(  //LISTO en DOT -- Version del Profe
                "\"command\"->\"'[|]'\";\n{}",
                commands.iter()
                    .map(|c| format!("\n\"'[|]'\"->\"{}\";\n{}\n", c.to_custom_dot().split('"').nth(1).unwrap_or("") , c.to_custom_dot()  )) //SE BUSCA QUE DECLARATION APUNTE A LOS HIJOS
                    .collect::<Vec<_>>()
                    .join("\n")
            ...
            ASTNode::Expression(expr) => expr.to_custom_dot(), // NO ESTOY SEGURO DE SI DEBE IMPLEMENTARSE en DOT
            ASTNode::Proc(_, _, _) | ASTNode::Type(_, _) | ASTNode::While(_, _) => {  // NO ESTOY SEGURO DE SI DEBE IMPLEMENTARSE en DOT
                "".to_string()
            }
        }
    }
